...
def _transfer(**kwargs):
    ...
    _, records = fetch_vendor_records("臺北市", vendor_category_code="6")
    ...
    data = records.merge(taipei_house_geocoded, on="address", how="left")

    tpgos_addresses = pd.Series(
        data.loc[data["taipei_house_lng"].isna(), "geocoding_address"].dropna().unique()
    )
    if tpgos_addresses.empty:
        geocoded = pd.DataFrame(columns=["geocoding_address", "tpgos_lng", "tpgos_lat"])
    else:
    ...
    data = data.merge(geocoded, on="geocoding_address", how="left")
    ...
    if data.empty:
        raise ValueError("No Taipei FDA logistics vendor records were aggregated.")
...
dag = CommonDag(
    ...
    dag_folder="food_safety_logistics_vendor_tpe",
)
