...
VALUES
('food_source', NULL, '{209,210}', '{"mode":"byParam","byParam":{"xParam":"district"}}', 'static', NULL, NULL, NULL, '臺北市政府產業發展局、新北市政府農業局', '顯示雙北有機農場分布。', '顯示臺北市取得有機驗證之有機農場與新北市有機農場之空間分布，並以行政區圖、矩形圖與橫向長條圖呈現各行政區有機農場數量。資料包含農場名稱、經營者、行政區、地址、驗證狀態、面積與聯絡資訊；地址可定位者使用地址座標，地號或無法定位者以行政區中心點加微小偏移顯示，作為有機農場與食安供應鏈節點盤點基礎。', '可用於掌握雙北有機農產品來源分布、食安供應鏈來源追蹤、產地與市場或物流節點套疊分析，以及農業資源與稽查服務配置。', '{https://data.taipei/dataset/detail?id=32aea2da-14a7-47b6-a687-57e29c1ad4a7,https://data.ntpc.gov.tw/datasets/fc30f585-66d9-4233-a65e-c650d177ebfe}', '{doit,ntpc}', NOW(), NOW(), 'two_d', 'SELECT x_axis, SUM(data)::int AS data FROM (SELECT district AS x_axis, COUNT(*)::int AS data FROM public.food_source_tpe WHERE district != '''' GROUP BY district UNION ALL SELECT district AS x_axis, COUNT(*)::int AS data FROM public.food_source_ntpe WHERE district != '''' GROUP BY district) d GROUP BY x_axis ORDER BY data DESC', NULL, 'metrotaipei'),
('food_source', NULL, '{209}', '{"mode":"byParam","byParam":{"xParam":"district"}}', 'static', NULL, NULL, NULL, '臺北市政府產業發展局', '顯示臺北市有機農場分布。', '顯示臺北市取得有機驗證之有機農場空間分布，並以行政區圖、矩形圖與橫向長條圖呈現各行政區有機農場數量。資料包含農場名稱、農友姓名、行政區、地址、認證字號、面積與食農教育等欄位；地址可定位者使用地址座標，無法定位者以行政區中心點加微小偏移顯示，作為有機農場與食安供應鏈節點盤點基礎。', '可用於掌握臺北市有機農產品來源分布、食安供應鏈來源追蹤、產地與市場或物流節點套疊分析，以及農業資源與稽查服務配置。', '{https://data.taipei/dataset/detail?id=32aea2da-14a7-47b6-a687-57e29c1ad4a7}', '{doit}', NOW(), NOW(), 'two_d', 'SELECT district AS x_axis, COUNT(*)::int AS data FROM public.food_source_tpe WHERE district != '''' GROUP BY district ORDER BY data DESC', NULL, 'taipei');
