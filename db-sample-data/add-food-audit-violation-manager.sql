...
VALUES (308, 'food_audit_violation', '食品抽驗結果統計')
ON CONFLICT (id) DO UPDATE
SET index = EXCLUDED.index,
    name = EXCLUDED.name;
...
('food_audit_violation', NULL, '{211}', '{"mode":"byParam","byParam":{"xParam":"district"}}', 'static', NULL, NULL, NULL, '臺北市政府衛生局', '顯示臺北市食品抽驗不合格單位行政區分布。', '顯示臺北市衛生局食品抽驗不合格清冊的空間分布，並以行政區圖、矩形圖與橫向長條圖呈現各行政區違規件數。資料包含業者名稱、地址、抽驗日期、專案名稱、分類、檢體名稱、檢驗結果與不符合規定原因；地圖點位以門牌資料與地址定位，無法精準定位者以行政區中心點加微小偏移顯示。', '可用於掌握臺北市食品抽驗違規熱點、安排衛生稽查複查，並與市場、物流與有機農場節點套疊分析。', '{https://data.taipei/dataset/detail?id=09a917a0-0fb5-47e1-957c-5f1268fba517}', '{doit}', NOW(), NOW(), 'two_d', 'SELECT district AS x_axis, COUNT(*)::int AS data FROM public.food_audit_violation_tpe WHERE district != '''' GROUP BY district ORDER BY data DESC', NULL, 'taipei');
