...
SET short_desc = '顯示雙北衛生局所屬服務據點類型統計。',
    ...
    query_type = 'two_d',
    query_chart = 'SELECT unnest(array[''衛生所'',''健康服務中心'',''衛生局'']) as x_axis, unnest(array[29,12,2]) as data',
    updated_at = NOW()
WHERE index = 'food_safety_health_office'
  AND city = 'metrotaipei';

UPDATE public.query_charts
SET short_desc = '顯示臺北市政府衛生局與健康服務中心類型統計。',
    long_desc = '顯示臺北市政府衛生局與各區健康服務中心之空間分布，並以圓餅圖、矩形圖與橫向長條圖呈現各類型據點數量；包含名稱、行政區、地址、電話與網站等資訊。點位優先使用臺北市門牌位置數值資料定位。',
    query_type = 'two_d',
    query_chart = 'SELECT unnest(array[''健康服務中心'',''衛生局'']) as x_axis, unnest(array[12,1]) as data',
    updated_at = NOW()
WHERE index = 'food_safety_health_office'
  AND city = 'taipei';

UPDATE public.component_charts
SET color = '{#1E88E5,#00A6A6,#4A90E2,#30B68F,#F5B041,#8E63C7,#D84C73,#7A8793,#2F7D6D,#C2573E,#AF4137}',
    types = '{TreemapChart,BarChart}',
    unit = 'NTU'
WHERE index = 'water_quality';

UPDATE public.query_charts
SET short_desc = '顯示雙北淨水場濁度比較。',
    long_desc = '顯示臺北自來水淨水場清水水質年度統計，以及臺灣自來水公司平均水質中關鍵字為新北市的淨水場最新測值；總覽以矩形圖與橫向長條圖比較各淨水場濁度(NTU)，並在地圖點位中提供 pH、濁度、自由有效餘氯、總硬度、總溶解固體量與大腸桿菌群等欄位。',
    query_type = 'two_d',
...
UPDATE public.query_charts
SET short_desc = '顯示臺北市淨水場濁度比較。',
    long_desc = '顯示臺北自來水淨水場清水水質年度統計；總覽以矩形圖與橫向長條圖比較各淨水場濁度(NTU)，並在地圖點位中提供 pH、濁度、自由有效餘氯、總硬度、總溶解固體量與大腸桿菌群等欄位。',
    query_type = 'two_d',
